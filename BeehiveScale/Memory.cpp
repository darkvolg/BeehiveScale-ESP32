...
void load_alerts(AlertSettings &a) {
  ...
  if (magic != EEPROM_MAGIC_ALERTS_VALUE) {
    // v5.0.67: алерт низкой батареи включён по умолчанию (3.6 В).
    // Раньше дефолт был 0.0 = выключено, и разряд банки проходил молча:
    // весы умирали в улье, а хозяин узнавал об этом через недели по тишине в TG.
    // 3.6 В ≈ 15% заряда — остаётся ~1-2 дня чтобы снять и зарядить.
    // Кто хочет отключить — ставит 0 на сайте, тогда magic запишется и это значение уцелеет.
    a.batLowV  = 3.6f;
    a.tempLow  = 0.0f;
    a.tempHigh = 0.0f;
    a.rtcEn    = true;
